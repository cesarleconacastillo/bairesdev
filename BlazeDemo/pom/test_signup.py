...
def test_singup_success(test_data: LoginData, page: Page):
    ...
    signup.enter_username(test_data.username + str(random.randrange(100, 10000)))
    signup.enter_password(test_data.password)
    signup.click_submit()
    page.on("dialog", lambda dialog: dialog.accept())
    
@pytest.mark.parametrize('test_data', csv_data.filter_on_test_case_id('TC-3'))
def test_singup_empty_username(test_data: LoginData, page: Page):
    signup = SignupPage(page)
    page.goto("/")
    signup.open_signup_modal()
    expect(signup.get_modal_name()).to_contain_text('Sign up')
...
@pytest.mark.parametrize('test_data', csv_data.filter_on_test_case_id('TC-3'))
def test_singup_empty_password(test_data: LoginData, page: Page):
    signup = SignupPage(page)
    page.goto("/")
    signup.open_signup_modal()
    expect(signup.get_modal_name()).to_contain_text('Sign up')
...
@pytest.mark.parametrize('test_data', csv_data.filter_on_test_case_id('TC-3'))
def test_singup_existing_user(test_data: LoginData, page: Page):
    signup = SignupPage(page)
    page.goto("/")
    signup.open_signup_modal()
    expect(signup.get_modal_name()).to_contain_text('Sign up')
